...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void StartRoll() //Roll은 각도처리 따로 함 이동값으로 판별해서 
    {
        if (v != 0 && h == 0) // 왼쪽 또는 오른쪽으로 이동할 때
        {
            Vector3 currentScale = transform.localScale;
            if (h < 0) // 왼쪽 이동
            {
        ...
        else if (v == 0 && h != 0) // 위 또는 아래로 이동할 때
        {
        ...
        else // 대각선 이동일 때
        {
            ...
            else if (h > 0 && v > 0) // 오른쪽 위 대각선
            {
                currentScale.x = Mathf.Abs(currentScale.x) * 1;//  스케일 그대로 설정
                animator.SetBool("IsAngle", true);
            }
            else if (h < 0 && v < 0) // 왼쪽 아래 대각선
            {
                animator.SetBool("IsSide", true);
                currentScale.x = Mathf.Abs(currentScale.x) * -1;// 스케일 반대로 설정
            }
            else if (h > 0 && v < 0) // 오른쪽 아래 대각선
            {
                animator.SetBool("IsSide", true);
                currentScale.x = Mathf.Abs(currentScale.x) * 1;//  스케일 그대로 설정
            }
